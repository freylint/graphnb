# LMPriestley's Notebook

This is a personal project to collect my personal knowledge and small utilities into an obsidian inspired jupyter notebook.

To view the interactive version of this project, please visit [lmpriestley.com](https://freyground.com). Sources available at [github.com/freylint/gbnotebook](https://www.github.com/freylint/gbnotebook).

## Table of Contents

- [License](#License)
- [Packages](#Packages)
  - [NBECS Interface - Notebook ECS interfaces](#NBECS-Interface---Notebook-ECS-interfaces)
  - [NBECS](#NBECS)
- [Articles](#Articles)
- [Notes](#Notes)
- [Images](#Images)
- [Homelab Setup](#Homelab-Setup)
  - [Hardware](#Hardware)
    - [Network](#Network)
    - [Homelab Server](#Homelab-Server)
    - [Thin Client](#Thin-Client)


## License

This project is licensed under the GNU Affero General Public License v3.0.
See [LICENSE](LICENSE) for the full text.

## Homelab Setup

### Overview

The homelab is built on a hybrid cloud architecture where local machines are managed through [OSTree](). The cloud machines are managed through [OSI Image]() deployments to [AWS Lightsail](). The secure tunnel for the hybrid cloud is provided via [Strongswan]().

Images are seperated into the following categories based on their expected execution environment:
- Bare Metal
  - Client - Nvidia Drivers
  - Client - AMDGPU
  - Server - AMDGPU
- Cloud
  - Gateway - Revproxy / LDAP / Tunnel

#### Software Stack / Configuration Management
All images are based on debian OSI Images. Local machines are deployed via Netboot / OSTree. Cloud images are managed through OSI Image upload to the cloud vendor.

##### Image Generation Script

Image generation is performed via [debootstrap](). Images are then converted to netboot / OSTree / Container formats and deployed seperately in the CD Environment.

See below for implementation details:


```typescript
// Homelab BaseImage Generation script

import {
  DEBIAN_RELEASE,
  DEBOOTSTRAP_VARIANT,
  DEBOOTSTRAP_MIRROR,
  LOCAL_DIST_DIR_SRV,
  LOCAL_DIST_DIR_CLIENT,
  LOCAL_DIST_DIR_GW,
  LOCAL_BUILD_DIR,
  runSyncCommand,
  runChrootCommand,
  ensureDir,
  isDirEmpty,
  clearDir,
  checkCommandAvailable,
} from './scripts/prelude';

import {existsSync} from 'fs';

/**
 * Builds the base Debian image using debootstrap.
 * 
 * The base image implements the functionality common to both client and server images.
 */
function buildBaseImage() {
    console.log('Syncing Debian min image for test build...');
    console.log('Debian release:', DEBIAN_RELEASE);
    console.log('Debootstrap variant:', DEBOOTSTRAP_VARIANT);
    console.log('Output directory:', LOCAL_BUILD_DIR);
    console.log('Mirror:', DEBOOTSTRAP_MIRROR);

    runSyncCommand('debootstrap', [
        '--variant=' + DEBOOTSTRAP_VARIANT,
        DEBIAN_RELEASE,
        LOCAL_BUILD_DIR,
        DEBOOTSTRAP_MIRROR,
    ]);

    console.log('Debootstrap command completed.');
}
/**
 * Build the client image inside the client chroot.
 */
function buildClientImage() {
    runChrootCommand(LOCAL_DIST_DIR_CLIENT, 'echo', ['Hello from chroot']); 
}

/**
 * Build the server image inside the server chroot.
 */
function buildServerImage() {
    runChrootCommand(LOCAL_DIST_DIR_SRV, 'echo', ['Hello from server chroot']);
}

/**
 * Build the gateway image inside the gateway chroot.
 */
function buildGatewayImage() {
    runChrootCommand(LOCAL_DIST_DIR_GW, 'echo', ['Hello from gateway chroot']);
    // Clean apt cache to reduce image size
    runChrootCommand(LOCAL_DIST_DIR_GW, 'apt-get', ['clean']);
    
}

// Ensure build environment is set up
checkCommandAvailable('debootstrap');
ensureDir(LOCAL_DIST_DIR_SRV);
clearDir(LOCAL_BUILD_DIR);

// Build base image
if (existsSync(LOCAL_BUILD_DIR) && !isDirEmpty(LOCAL_BUILD_DIR)) {
    console.log('Skipping debootstrap because target directory already exists and is non-empty:', LOCAL_BUILD_DIR);
} else {
    buildBaseImage();
    // Copy base image to client directory for client image build
    ensureDir(LOCAL_DIST_DIR_CLIENT);
    runSyncCommand('rsync', ['-a', LOCAL_BUILD_DIR, LOCAL_DIST_DIR_CLIENT]);
    // Copy base image to server directory for server image build
    ensureDir(LOCAL_DIST_DIR_SRV);
    runSyncCommand('rsync', ['-a', LOCAL_BUILD_DIR, LOCAL_DIST_DIR_SRV]);
    // Copy base image to gateway directory for gateway image build
    ensureDir(LOCAL_DIST_DIR_GW);
    runSyncCommand('rsync', ['-a', LOCAL_BUILD_DIR, LOCAL_DIST_DIR_GW]);
}

// Build client and server images
buildClientImage();
buildServerImage();
buildGatewayImage();

```

<details class="output-spoiler">
<summary>Output</summary>

```text
Syncing Debian min image for test build...
Debian release: testing
Debootstrap variant: minbase
Output directory: /workspaces/graphnb/build/base/
Mirror: http://deb.debian.org/debian

> debootstrap --variant=minbase testing /workspaces/graphnb/build/base/ http://deb.debian.org/debian
I: Retrieving InRelease 
I: Checking Release signature
I: Valid Release signature (key id B8E5F13176D2A7A75220028078DBA3BC47EF2265)
I: Retrieving Packages 
I: Validating Packages 
I: Resolving dependencies of required packages...
I: Resolving dependencies of base packages...
I: Checking component main on http://deb.debian.org/debian...
I: Retrieving apt 3.2.0
I: Validating apt 3.2.0
I: Retrieving base-files 14
I: Validating base-files 14
I: Retrieving base-passwd 3.6.8
I: Validating base-passwd 3.6.8
I: Retrieving bash 5.3-2
I: Validating bash 5.3-2
I: Retrieving bsdutils 1:2.42-4
I: Validating bsdutils 1:2.42-4
I: Retrieving coreutils 9.10-1
I: Validating coreutils 9.10-1
I: Retrieving dash 0.5.12-12
I: Validating dash 0.5.12-12
I: Retrieving debconf 1.5.92
I: Validating debconf 1.5.92
I: Retrieving debian-archive-keyring 2025.1
I: Validating debian-archive-keyring 2025.1
I: Retrieving debianutils 5.23.2
I: Validating debianutils 5.23.2
I: Retrieving diffutils 1:3.12-1
I: Validating diffutils 1:3.12-1
I: Retrieving dpkg 1.23.7
I: Validating dpkg 1.23.7
I: Retrieving findutils 4.10.0-3
I: Validating findutils 4.10.0-3
I: Retrieving gcc-16-base 16-20260322-1
I: Validating gcc-16-base 16-20260322-1
I: Retrieving grep 3.12-1
I: Validating grep 3.12-1
I: Retrieving gzip 1.13-1
I: Validating gzip 1.13-1
I: Retrieving hostname 3.25
I: Validating hostname 3.25
I: Retrieving init-system-helpers 1.69
I: Validating init-system-helpers 1.69
I: Retrieving libacl1 2.3.2-3
I: Validating libacl1 2.3.2-3
I: Retrieving libapt-pkg7.0 3.2.0
I: Validating libapt-pkg7.0 3.2.0
I: Retrieving libattr1 1:2.5.2-4
I: Validating libattr1 1:2.5.2-4
I: Retrieving libaudit-common 1:4.1.2-1
I: Validating libaudit-common 1:4.1.2-1
I: Retrieving libaudit1 1:4.1.2-1+b1
I: Validating libaudit1 1:4.1.2-1+b1
I: Retrieving libblkid1 2.42-4
I: Validating libblkid1 2.42-4
I: Retrieving libbsd0 0.12.2-2+b1
I: Validating libbsd0 0.12.2-2+b1
I: Retrieving libbz2-1.0 1.0.8-6+b1
I: Validating libbz2-1.0 1.0.8-6+b1
I: Retrieving libc-bin 2.42-15
I: Validating libc-bin 2.42-15
I: Retrieving libc-gconv-modules-extra 2.42-15
I: Validating libc-gconv-modules-extra 2.42-15
I: Retrieving libc6 2.42-15
I: Validating libc6 2.42-15
I: Retrieving libcap-ng0 0.9.3-1
I: Validating libcap-ng0 0.9.3-1
I: Retrieving libcrypt1 1:4.5.1-1+b1
I: Validating libcrypt1 1:4.5.1-1+b1
I: Retrieving libdb5.3t64 5.3.28+dfsg2-11+b1
I: Validating libdb5.3t64 5.3.28+dfsg2-11+b1
I: Retrieving libdebconfclient0 0.282+b2
I: Validating libdebconfclient0 0.282+b2
I: Retrieving libgcc-s1 16-20260322-1
I: Validating libgcc-s1 16-20260322-1
I: Retrieving libgmp10 2:6.3.0+dfsg-5+b1
I: Validating libgmp10 2:6.3.0+dfsg-5+b1
I: Retrieving libhogweed6t64 3.10.2-1+b1
I: Validating libhogweed6t64 3.10.2-1+b1
I: Retrieving liblz4-1 1.10.0-10
I: Validating liblz4-1 1.10.0-10
I: Retrieving liblzma5 5.8.3-1
I: Validating liblzma5 5.8.3-1
I: Retrieving libmd0 1.1.0-2+b2
I: Validating libmd0 1.1.0-2+b2
I: Retrieving libmount1 2.42-4
I: Validating libmount1 2.42-4
I: Retrieving libnettle8t64 3.10.2-1+b1
I: Validating libnettle8t64 3.10.2-1+b1
I: Retrieving libpam-modules 1.7.0-5+b1
I: Validating libpam-modules 1.7.0-5+b1
I: Retrieving libpam-modules-bin 1.7.0-5+b1
I: Validating libpam-modules-bin 1.7.0-5+b1
I: Retrieving libpam-runtime 1.7.0-5
I: Validating libpam-runtime 1.7.0-5
I: Retrieving libpam0g 1.7.0-5+b1
I: Validating libpam0g 1.7.0-5+b1
I: Retrieving libpcre2-8-0 10.46-1+b1
I: Validating libpcre2-8-0 10.46-1+b1
I: Retrieving libseccomp2 2.6.0-2+b1
I: Validating libseccomp2 2.6.0-2+b1
I: Retrieving libselinux1 3.10-1
I: Validating libselinux1 3.10-1
I: Retrieving libsemanage-common 3.10-1
I: Validating libsemanage-common 3.10-1
I: Retrieving libsemanage2 3.10-1
I: Validating libsemanage2 3.10-1
I: Retrieving libsepol2 3.10-1
I: Validating libsepol2 3.10-1
I: Retrieving libsmartcols1 2.42-4
I: Validating libsmartcols1 2.42-4
I: Retrieving libssl3t64 3.6.2-1
I: Validating libssl3t64 3.6.2-1
I: Retrieving libstdc++6 16-20260322-1
I: Validating libstdc++6 16-20260322-1
I: Retrieving libsystemd0 260.1-1
I: Validating libsystemd0 260.1-1
I: Retrieving libtinfo6 6.6+20251231-1
I: Validating libtinfo6 6.6+20251231-1
I: Retrieving libudev1 260.1-1
I: Validating libudev1 260.1-1
I: Retrieving libuuid1 2.42-4
I: Validating libuuid1 2.42-4
I: Retrieving libxxhash0 0.8.3-2+b1
I: Validating libxxhash0 0.8.3-2+b1
I: Retrieving libzstd1 1.5.7+dfsg-3+b1
I: Validating libzstd1 1.5.7+dfsg-3+b1
I: Retrieving login 1:4.16.0-2+really2.42-4
I: Validating login 1:4.16.0-2+really2.42-4
I: Retrieving login.defs 1:4.19.3-2
I: Validating login.defs 1:4.19.3-2
I: Retrieving mawk 1.3.4.20260302-1
I: Validating mawk 1.3.4.20260302-1
I: Retrieving mount 2.42-4
I: Validating mount 2.42-4
I: Retrieving ncurses-base 6.6+20251231-1
I: Validating ncurses-base 6.6+20251231-1
I: Retrieving ncurses-bin 6.6+20251231-1
I: Validating ncurses-bin 6.6+20251231-1
I: Retrieving openssl-provider-legacy 3.6.2-1
I: Validating openssl-provider-legacy 3.6.2-1
I: Retrieving passwd 1:4.19.3-2
I: Validating passwd 1:4.19.3-2
I: Retrieving perl-base 5.40.1-7+b1
I: Validating perl-base 5.40.1-7+b1
I: Retrieving sed 4.9-3
I: Validating sed 4.9-3
I: Retrieving sqv 1.3.0-5+b1
I: Validating sqv 1.3.0-5+b1
I: Retrieving sysvinit-utils 3.18-1
I: Validating sysvinit-utils 3.18-1
I: Retrieving tar 1.35+dfsg-4
I: Validating tar 1.35+dfsg-4
I: Retrieving tzdata 2026a-3
I: Validating tzdata 2026a-3
I: Retrieving util-linux 2.42-4
I: Validating util-linux 2.42-4
I: Retrieving zlib1g 1:1.3.dfsg+really1.3.1-3
I: Validating zlib1g 1:1.3.dfsg+really1.3.1-3
I: Chosen extractor for .deb packages: dpkg-deb
I: Extracting apt...
I: Extracting base-files...
I: Extracting base-passwd...
I: Extracting bash...
I: Extracting bsdutils...
I: Extracting coreutils...
I: Extracting dash...
I: Extracting debconf...
I: Extracting debian-archive-keyring...
I: Extracting debianutils...
I: Extracting diffutils...
I: Extracting dpkg...
I: Extracting findutils...
I: Extracting gcc-16-base...
I: Extracting grep...
I: Extracting gzip...
I: Extracting hostname...
I: Extracting init-system-helpers...
I: Extracting libacl1...
I: Extracting libapt-pkg7.0...
I: Extracting libattr1...
I: Extracting libaudit-common...
I: Extracting libaudit1...
I: Extracting libblkid1...
I: Extracting libbsd0...
I: Extracting libbz2-1.0...
I: Extracting libc-bin...
I: Extracting libc-gconv-modules-extra...
I: Extracting libc6...
I: Extracting libcap-ng0...
I: Extracting libcrypt1...
I: Extracting libdb5.3t64...
I: Extracting libdebconfclient0...
I: Extracting libgcc-s1...
I: Extracting libgmp10...
I: Extracting libhogweed6t64...
I: Extracting liblz4-1...
I: Extracting liblzma5...
I: Extracting libmd0...
I: Extracting libmount1...
I: Extracting libnettle8t64...
I: Extracting libpam-modules...
I: Extracting libpam-modules-bin...
I: Extracting libpam-runtime...
I: Extracting libpam0g...
I: Extracting libpcre2-8-0...
I: Extracting libseccomp2...
I: Extracting libselinux1...
I: Extracting libsemanage-common...
I: Extracting libsemanage2...
I: Extracting libsepol2...
I: Extracting libsmartcols1...
I: Extracting libssl3t64...
I: Extracting libstdc++6...
I: Extracting libsystemd0...
I: Extracting libtinfo6...
I: Extracting libudev1...
I: Extracting libuuid1...
I: Extracting libxxhash0...
I: Extracting libzstd1...
I: Extracting login...
I: Extracting login.defs...
I: Extracting mawk...
I: Extracting mount...
I: Extracting ncurses-base...
I: Extracting ncurses-bin...
I: Extracting openssl-provider-legacy...
I: Extracting passwd...
I: Extracting perl-base...
I: Extracting sed...
I: Extracting sqv...
I: Extracting sysvinit-utils...
I: Extracting tar...
I: Extracting tzdata...
I: Extracting util-linux...
I: Extracting zlib1g...
W: Failure trying to run: chroot "/workspaces/graphnb/build/base" mount -t proc proc /proc
W: See /workspaces/graphnb/build/base/debootstrap/debootstrap.log for details
I: Installing core packages...
I: Unpacking required packages...
I: Unpacking apt...
I: Unpacking base-files...
I: Unpacking base-passwd...
I: Unpacking bash...
I: Unpacking bsdutils...
I: Unpacking coreutils...
I: Unpacking dash...
I: Unpacking debconf...
I: Unpacking debian-archive-keyring...
I: Unpacking debianutils...
I: Unpacking diffutils...
I: Unpacking dpkg...
I: Unpacking findutils...
I: Unpacking gcc-16-base:amd64...
I: Unpacking grep...
I: Unpacking gzip...
I: Unpacking hostname...
I: Unpacking init-system-helpers...
I: Unpacking libacl1:amd64...
I: Unpacking libapt-pkg7.0:amd64...
I: Unpacking libattr1:amd64...
I: Unpacking libaudit-common...
I: Unpacking libaudit1:amd64...
I: Unpacking libblkid1:amd64...
I: Unpacking libbsd0:amd64...
I: Unpacking libbz2-1.0:amd64...
I: Unpacking libc-bin...
I: Unpacking libc-gconv-modules-extra:amd64...
I: Unpacking libc6:amd64...
I: Unpacking libcap-ng0:amd64...
I: Unpacking libcrypt1:amd64...
I: Unpacking libdb5.3t64:amd64...
I: Unpacking libdebconfclient0:amd64...
I: Unpacking libgcc-s1:amd64...
I: Unpacking libgmp10:amd64...
I: Unpacking libhogweed6t64:amd64...
I: Unpacking liblz4-1:amd64...
I: Unpacking liblzma5:amd64...
I: Unpacking libmd0:amd64...
I: Unpacking libmount1:amd64...
I: Unpacking libnettle8t64:amd64...
I: Unpacking libpam-modules:amd64...
I: Unpacking libpam-modules-bin...
I: Unpacking libpam-runtime...
I: Unpacking libpam0g:amd64...
I: Unpacking libpcre2-8-0:amd64...
I: Unpacking libseccomp2:amd64...
I: Unpacking libselinux1:amd64...
I: Unpacking libsemanage-common...
I: Unpacking libsemanage2:amd64...
I: Unpacking libsepol2:amd64...
I: Unpacking libsmartcols1:amd64...
I: Unpacking libssl3t64:amd64...
I: Unpacking libstdc++6:amd64...
I: Unpacking libsystemd0:amd64...
I: Unpacking libtinfo6:amd64...
I: Unpacking libudev1:amd64...
I: Unpacking libuuid1:amd64...
I: Unpacking libxxhash0:amd64...
I: Unpacking libzstd1:amd64...
I: Unpacking login...
I: Unpacking login.defs...
I: Unpacking mawk...
I: Unpacking mount...
I: Unpacking ncurses-base...
I: Unpacking ncurses-bin...
I: Unpacking openssl-provider-legacy...
I: Unpacking passwd...
I: Unpacking perl-base...
I: Unpacking sed...
I: Unpacking sqv...
I: Unpacking sysvinit-utils...
I: Unpacking tar...
I: Unpacking tzdata...
I: Unpacking util-linux...
I: Unpacking zlib1g:amd64...
I: Configuring required packages...
I: Configuring libc-gconv-modules-extra:amd64...
I: Configuring login.defs...
I: Configuring debian-archive-keyring...
I: Configuring gcc-16-base:amd64...
I: Configuring libaudit-common...
I: Configuring libsemanage-common...
I: Configuring debconf...
I: Configuring tzdata...
I: Configuring ncurses-base...
I: Configuring init-system-helpers...
I: Configuring libgcc-s1:amd64...
I: Configuring libc6:amd64...
I: Configuring libdb5.3t64:amd64...
I: Configuring libudev1:amd64...
I: Configuring libmd0:amd64...
I: Configuring libxxhash0:amd64...
I: Configuring libattr1:amd64...
I: Configuring sysvinit-utils...
I: Configuring debianutils...
I: Configuring mawk...
I: Configuring libdebconfclient0:amd64...
I: Configuring base-files...
I: Configuring libbz2-1.0:amd64...
I: Configuring libbsd0:amd64...
I: Configuring libblkid1:amd64...
I: Configuring libstdc++6:amd64...
I: Configuring dash...
I: Configuring libtinfo6:amd64...
I: Configuring bash...
I: Configuring libzstd1:amd64...
I: Configuring liblzma5:amd64...
I: Configuring liblz4-1:amd64...
I: Configuring libc-bin...
I: Configuring ncurses-bin...
I: Configuring libacl1:amd64...
I: Configuring libsmartcols1:amd64...
I: Configuring zlib1g:amd64...
I: Configuring libcrypt1:amd64...
I: Configuring diffutils...
I: Configuring libseccomp2:amd64...
I: Configuring libsystemd0:amd64...
I: Configuring perl-base...
I: Configuring openssl-provider-legacy...
I: Configuring hostname...
I: Configuring libcap-ng0:amd64...
I: Configuring libnettle8t64:amd64...
I: Configuring libgmp10:amd64...
I: Configuring libaudit1:amd64...
I: Configuring gzip...
I: Configuring libuuid1:amd64...
I: Configuring libsepol2:amd64...
I: Configuring libpcre2-8-0:amd64...
I: Configuring libhogweed6t64:amd64...
I: Configuring sqv...
I: Configuring bsdutils...
I: Configuring grep...
I: Configuring libssl3t64:amd64...
I: Configuring libselinux1:amd64...
I: Configuring libpam0g:amd64...
I: Configuring sed...
I: Configuring findutils...
I: Configuring libmount1:amd64...
I: Configuring libsemanage2:amd64...
I: Configuring libapt-pkg7.0:amd64...
I: Configuring base-passwd...
I: Configuring apt...
I: Configuring tar...
I: Configuring libpam-modules-bin...
I: Configuring coreutils...
I: Configuring dpkg...
I: Configuring mount...
I: Configuring libpam-modules:amd64...
I: Configuring passwd...
I: Configuring libpam-runtime...
I: Configuring login...
I: Configuring util-linux...
I: Configuring libc-bin...
I: Unpacking the base system...
I: Base system installed successfully.

Debootstrap command completed.

> rsync -a /workspaces/graphnb/build/base/ /workspaces/graphnb/dist/client/

> rsync -a /workspaces/graphnb/build/base/ /workspaces/graphnb/dist/server/

> rsync -a /workspaces/graphnb/build/base/ /workspaces/graphnb/dist/gateway/
Running in chroot shell: /bin/sh -c echo 'Hello from chroot'

> chroot /workspaces/graphnb/dist/client/ /bin/sh -c echo 'Hello from chroot'
Hello from chroot

Running in chroot shell: /bin/sh -c echo 'Hello from server chroot'

> chroot /workspaces/graphnb/dist/server/ /bin/sh -c echo 'Hello from server chroot'
Hello from server chroot

Running in chroot shell: /bin/sh -c echo 'Hello from gateway chroot'

> chroot /workspaces/graphnb/dist/gateway/ /bin/sh -c echo 'Hello from gateway chroot'
Hello from gateway chroot

Running in chroot shell: /bin/sh -c apt-get clean

> chroot /workspaces/graphnb/dist/gateway/ /bin/sh -c apt-get clean
```
</details>


## Articles
## TODO / Help Wanted
